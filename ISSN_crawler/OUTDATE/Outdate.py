                    ...
                    try:
                        ...
                        link_elements = WebDriverWait(driver, 10).until(
                            EC.presence_of_all_elements_located(
                                (By.CSS_SELECTOR, "#CataLogContent span.name a, .row a"))
                        )
                        ...
                        logging.info(f"第 {i + 1} 期找到 {len(link_elements)} 篇文章")
                ...
                if all_links:
                    ...
                    with open(output_file, 'w', encoding='utf-8') as f:
                        for link in all_links:
                            f.write(link + '\n')
                    logging.info(f"保存 {len(all_links)} 个链接到文件: {output_file}")
                else:
                    logging.warning(f"{journal_name} {year} 年未找到任何文章链接")
